...
/**
 * Reverse the direction of directed edges in
 * a digraph. Basically, copies the graph but
 * changes direction of edges from original
 *
 */
public Graph reverse() {
        Graph g = new Graph(this.numNodes);
        for (Vertex v : this) {
                for (Edge e : v.Adj) {
                        int from = e.From.name;
                        int to = e.To.name;
                        int w = e.Weight;
                        g.addDirectedEdge(to, from, w);
                }
        }
        return g;
}
...
/**
 * Gets and returns a specific vertex
 *
