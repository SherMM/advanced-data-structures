...
public Edge getMinWeightIncomingEdge() {

        Edge min = this.revAdj.get(0);
        for (int i = 1; i < this.revAdj.size(); i++) {
                Edge edge = this.revAdj.get(i);
                if (edge.cost < min.cost) {
                        min = edge;
                }
        }
        return min;
